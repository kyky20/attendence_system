...
<body>
  ...
  <div class="summary-strip">
    ...
    <div class="sum-pill">
      <div class="val" style="color:#0d6efd;">{{ $totalSks ?? '0' }}</div>
  ...
  <div class="p-3" style="max-width:640px;margin:auto;">
      ...
      <div class="section-card-body">
        ...
        <div class="info-row">
          ...
          <span class="info-value">
            <span
              class="badge text-bg-{{ $mahasiswa->status === 'Aktif' ? 'success' : 'warning' }} rounded-pill px-3">{{ $mahasiswa->status ?? 'N/A' }}</span>
          </span>
        </div>

        <div class="info-row">
          <span class="info-label">Total SKS</span>
          <span class="info-value">{{ $totalSks ?? '0' }} SKS</span>
        </div>
      </div>
    </div>
      ...
      <div class="section-card-body">
        ...
        <div class="d-flex align-items-center gap-3 mb-3 pb-3" style="border-bottom:1px solid #f0f0f0;">
          <div class="circle-wrap">
            <svg width="110" height="110">
              ...
              <circle cx="55" cy="55" r="48" fill="none" stroke="var(--maroon)" stroke-width="8"
                stroke-dasharray="301.6" stroke-dashoffset="{{ 301.6 * (1 - $persentaseKehadiran / 100) }}"
          ...
          <div>
            ...
            <div style="font-size:1.5rem;font-weight:700;color:#1a1a2e;line-height:1;">
              {{ round($persentaseKehadiran) }}%</div>
            <div style="font-size:.75rem;color:#888;margin-top:.2rem;">{{ $totalHadir }} / {{ $totalPertemuan }}
              pertemuan hadir</div>
            <div class="d-flex gap-2 mt-2 flex-wrap">
              <span
                style="font-size:.72rem;background:rgba(25,135,84,0.1);color:#198754;padding:.2rem .6rem;border-radius:20px;">Hadir:
                {{ $totalHadir }}</span>
              <span
                style="font-size:.72rem;background:rgba(253,126,20,0.1);color:#fd7e14;padding:.2rem .6rem;border-radius:20px;">Izin:
                {{ $totalIzin }}</span>
              <span
                style="font-size:.72rem;background:rgba(220,53,69,0.1);color:#dc3545;padding:.2rem .6rem;border-radius:20px;">Alpha:
                {{ $totalAlpha }}</span>
            </div>
          </div>
        </div>
    ...
    <div class="section-card mb-3">
      ...
      <div class="section-card-body">
        <p style="font-size:.82rem;color:#888;margin-bottom:.8rem;">Anda akan keluar dari sesi ini dan diarahkan ke
          halaman login.</p>
  ...
  <script>
    ...
    async function saveProfile() {
      ...
      try {
        const res = await fetch('/mahasiswa/profile', {
          method: 'PUT',
          headers: {
          ...
          body: JSON.stringify({ name: nama, no_hp: hp, alamat: alamat }),
        });
        if (res.redirected) {
          // Server redirected (success), reload to reflect changes
        ...
        // Fallback: if the controller returns redirect, fetch follows it
        modalEditProfil.hide();
        showToast('Profil berhasil diperbarui!', 'success');
        setTimeout(() => location.reload(), 800);
      }
    ...
    async function savePassword() {
      ...
      if (!lama || !baru || !konfirmasi) { showToast('Semua field wajib diisi!', 'danger'); return; }
      if (baru !== konfirmasi) { showToast('Konfirmasi password tidak cocok!', 'danger'); return; }

      try {
        const res = await fetch('/mahasiswa/profile/password', {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content || '',
